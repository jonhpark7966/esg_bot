...
class RetrieverHandler:
    ...
    def add_documents(self, doc_summaries, doc_contents, source, company_name, year):
        ...
        summary_docs = [
            Document(page_content=s, metadata={self.id_key: doc_ids[i], "source_url": source, "company_name": company_name, "year": str(year)})  # to string
            for i, s in enumerate(doc_summaries)
        ]
        self.retriever.vectorstore.add_documents(summary_docs)  # 요약 문서를 벡터 저장소에 추가
        self.retriever.docstore.mset(list(zip(doc_ids, doc_contents, strict=False)))  # 문서 내용을 문서 저장소에 추가
